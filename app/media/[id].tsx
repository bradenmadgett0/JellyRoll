    ...
    <View style={styles.container}>
      ...
      >
        ...
        >
                ...
                <View>
                  ...
                  >
                    <Ionicons
                      name="play"
                      size={22}
                      color={styles.playButtonText.color}
                    />
                    <Text style={styles.playButtonText}>
                      {isResumable
                        ? `Resume · ${formatResumeTime(positionTicks)}`
                        : "Play"}
                    </Text>
                  </TouchableOpacity>
                  {isResumable && (
          ...
          {/* Genres */}
          {genreText && <Text style={styles.genres}>{genreText}</Text>}
          ...
          {isSeries && seasonGroups.length > 0 && (
            <Animated.View entering={FadeInDown.duration(400).delay(500)}>
              ...
              <EpisodeList
                ...
                accentColor={styles.jellyfinColor.color as string}
              />
            </Animated.View>
          )}
            ...
            <Animated.View entering={FadeInDown.duration(400).delay(600)}>
              ...
              {item.MediaSources.map((source) => (
                <View key={source.Id} style={styles.mediaInfoCard}>
                  ...
                  {source.MediaStreams?.filter((s) => s.Type === "Video").map(
                    (stream) => (
                      <Text key={stream.Index} style={styles.mediaStreamText}>
                        Video:{" "}
                        {stream.DisplayTitle ??
                          `${stream.Codec} ${stream.Width}×${stream.Height}`}
                      </Text>
                    ),
                  )}
                  {source.MediaStreams?.filter((s) => s.Type === "Audio").map(
                    (stream) => (
                      <Text key={stream.Index} style={styles.mediaStreamText}>
                        Audio:{" "}
                        {stream.DisplayTitle ??
