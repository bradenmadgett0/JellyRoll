...
export default function RadarrDetailScreen() {
    ...
    const bannerUrl = numericId !== undefined ? getImageUrl(numericId, 'fanart') : null;
    const posterUrl = numericId !== undefined ? getImageUrl(numericId, 'poster') : null;
    ...
    const runtimeText = runtimeHours > 0 ? `${runtimeHours}h ${runtimeMins}m` : `${runtimeMins}m`;
                ...
                {movie.movieFile && (
                    <Animated.View entering={FadeInDown.duration(400).delay(400)} style={styles.section}>
                        ...
                        <View style={styles.fileCard}>
                            <Text style={styles.fileName}>{movie.movieFile.relativePath}</Text>
                            <View style={styles.fileInfoRow}>
                                <View style={styles.fileChip}>
                                    <Text style={styles.fileChipText}>{formatSize(movie.movieFile.size)}</Text>
                                </View>
                                {movie.movieFile.quality?.quality?.name && (
                                    <View style={styles.fileChip}>
                                        <Text style={styles.fileChipText}>{movie.movieFile.quality.quality.name}</Text>
                                    </View>
                                )}
                                {movie.movieFile.mediaInfo?.resolution && (
                                    <View style={styles.fileChip}>
                                        <Text style={styles.fileChipText}>{movie.movieFile.mediaInfo.resolution}</Text>
                                    </View>
                                )}
                                {movie.movieFile.mediaInfo?.videoCodec && (
                                    <View style={styles.fileChip}>
                                        <Text style={styles.fileChipText}>{movie.movieFile.mediaInfo.videoCodec}</Text>
                                    </View>
                                )}
                            </View>
                        </View>
